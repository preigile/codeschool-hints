...
## 2.7 Строительство View Next III
...
## 2.8 Сортировка по дате

_Задание._
Наши appointments в настоящее время рендерятся давольно бессистемно. Dr. Goodparts очень хронологический, поэтому встречи всегда должны быть отсортированы по дате.    
Example:
```javascript
...
_Решение._
Создади функцию cancelledCount, подсчитывающую количество отменённых встреч:
```javascript
var Appointments = Backbone.Collection.extend({
