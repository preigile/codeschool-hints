...
_Задание._
Мы услышали сообщения от путешественников, что иногда они нажимают на кнопку, но ничего не происходит, а через минуту они получат сообщение: "There was a problem fetching the latest photos. Please try again". Обновим запрос $.ajax, установив timeout в 3 секунды, чтобы предотвратить это.
...
_Задание._
Всё работает отлично, но можно сделать ещё лучше. Пока сервер отвечает, путешественник не видит никаких признаков, что что-то происходит. Было добавленно немного специальных стилей для учета этого. Перед отправлением команды ajax, добавим класс is-fetching в элемент #tour, а затем удалим этот класс после завершения запроса.
