...
Исапользуем $.Deferred() для создания своей собственной перспективы (установим с ним переменную promise), и употребим функцию .resolve() для передачи результата в коллбек .done().   
...
Метод deferred.resolve() решает объект Deferred и вызавает любой коллбек done с переданными аргументами. В качестве необязательных аргументов может выступать что угодно, что передалось в коллбек done.
...
_Пояснение._
Метод deferred.reject() подобен deferred.resolve(), но в отличае от resolve, reject не решает, а отклоняет объект Deferred и вызавает любой коллбек fail с переданными аргументами. 
...
_Задание._
Имеем 2 вызова Ajax, которые использованы для визуализации html страницы. Праблема в том, что, поскольку данные возвращаются в разное время, html мерцает и элементы отображаются в разных порядках.   
