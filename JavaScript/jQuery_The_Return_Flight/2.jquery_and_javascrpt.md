...
Затем, вызовем метод init в объекте tour изнутри функции $(document).ready. Наконец, переместим существующий код для загрузки на $(document).ready внутри level1.js в наш новый метод init. С функциональной точки зрения ничего не должно измениться, просто теперь код переместится в объект tour.   
level1.js:
```javascript
