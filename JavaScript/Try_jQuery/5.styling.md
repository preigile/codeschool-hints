...
_Задание._
Анимацие не хватает скорости. Сделаем её загрузку быстрее до  200ms, используя условное обозначение fast.
...
_Задание._
Обновим наш обработчик mouseleave, удалив цену за ночь, используя немного стиля. Анимируем цену свойствами top и opacity равным 0, когда курсор покидает элемент .tour, так, чтобы это было быстро. 

_Решение._
В обработчк события mouseleave поместим метод animate в элементы .per-night так, чтобы при переходе из каждого .tour на другие части страницы, цена быстро опускалась и становилась полостью прозрачной:
```javascript
$(document).ready(function () {
    $('.tour').on('mouseenter', function () {
        $(this).addClass('highlight');
