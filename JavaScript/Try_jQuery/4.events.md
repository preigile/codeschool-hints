...
_Решение._
Для начала в качестве аргумента функции обратного вызова обработчика событий элементов .see-photos поместим аргумент объект event. Затем вызовем на его свойство stopPropagation(), не допускающее вызова другого обработчика:
```javascript
$(document).ready(function () {
...
_Пояснение._
Метод [event.stopPropagation()](http://api.jquery.com/event.stopPropagation/) предотвращает подъёма события в дереве DOM, предотвращая любые родительские обработчики от получения уведомления о событии. Не принимает никаких аргументов.
