...
Пока мы думаем об Ember Data, преобразуем также наш массив CONTACTS.   
    ...
    7) Путь contact всё ещё ищет модели тоже используя старые закрепления! Обновить его для работы с повой настройкой либо изменив модель, либо используя по умолчанию.   
...
Пришло время добавить немного отзывов на нашу продукцию! Люди хотят знать, действительно ли всё работает.   
    ...
    3) Добавить ассоциации для объекта product в review. Помнить, что у продукта может быть много отзывов. Будим загружать эту ассоциацию асинхронно.       
...
_Решение._
Добавим модель и закрепление для Review, содержащего id и text, а затем объединим его с Product посредством добавления новы свойств, включающих id в закреплении, а также DS.hasMany() с асинхронной загрузкой для модели Product и DS.belongsTo() для модели Review. В HTML добавим вывод всех соответствующих отзывов или их отсутствия:   
app.js:
```javascript
App.Product = DS.Model.extend({
    title: DS.attr('string'),
...
Какую ещё ассоциацию можно добавить? К примеру, кто изготовил эти прекрасные продукты?     
    ...
    4) Обновить закрепление product для ссылку на контакт ремесленника для каждого продукта.    
