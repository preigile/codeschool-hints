...
_Пояснение._
Маршрут может быть динамически меняющимся, в зависимости от конкретных свойств. Для этого в моделе возвращаем объект, в котором ищется (findBy()) необходимое свойство, а в Router.map() в качестве значения свойства path, определяющего маршрут, используется :имяКонкретногоСвойства. Чтобы link-to знал, что используется, применим значение this, которое укажет на определённый в Router URL.
