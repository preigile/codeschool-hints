...
_Задание._
Наша страница About не достаточно хороша, чтобы удовлетворить любопытство по отношению к владельцам сайта. Они на самом деле Fire Sprites? Не плохо бы создать страницу Contacts, содержащую список всех Fire Sprites.   
...
Кажется немного странным, что у нас есть страницы About и Contacts. Может быть можно объединить их? Мы могли бы предоставить общую информацию на одной странице, но оставить список контактов с отдельными страницами для каждого.   
    ...
    3) Полностью удалить путь about. Мы собираемся переместить его в другое место. Также удалить ссылку c главной страницы на него.    
...
_Решение._
сделаем страницы about и contact вложенными в contacts, так, чтобы путь contact соответствовал свойству name, а about располагался на /index:
app.js: 
```javascript
App.Router.map(function () {
    this.route('credits', { path: '/thanks' });
...
_Пояснение._
В этот раз, вместо текстовой заглушки, на "главную" страницу contacts была помещена другая страница. 