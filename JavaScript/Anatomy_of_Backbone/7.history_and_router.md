...
_Задание._
Отлично, но наш AppRouter, на самом деле, не делает ничего...    
...
_Пояснение._
[History](http://backbonejs.ru/#History) служит глобальным роутером и чтобы его запустить используют метод [start()](http://backbonejs.ru/#History-start), в котором можно указать, что мы хотим использовать поддержку pushState из HTML5, установив его в true.

## 7.5 Навигация

_Задание._
Протестируем приложение и убедимся, что всё работает.   
Был создан экземпляр AppRouter. Используя функцию navigate, пройдём к appointments/1 и удостоверимся, что вызывается маршрут.
...
_Задание._
Мы близки к завершению! Прежде чем закончить, используем Router для небольшой чистки нашего приложения.    
