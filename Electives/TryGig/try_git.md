...
_Пояснение._
Команда `git checkout` переключает Git с одной ветки на другую. Она работает и без "--", но в данном случае командной строке обещается, что после "--" нет больше опций. Таким образом, если имеется ветвь с именем octocat.txt, Git всё ещё будет возвращать файл, вместо переключения файла на ветку с похожим именем.
...
_Зодание._
Когда разработчики работают над характеристиками или багами ошибками, они часто создают копии (aka. branch) кода, где можно делать обособленные коммиты. Затем новые ветви можно объединять с главной ветвью master.
...
_Пояснение._
Возможность удаления одного или всех файлов весьма полезна. Но иногда можно удалить всю папку целиком. В этом случае можно использовать рекурсивную опцию на `git rm` - `git rm -r folder_of_cats`. Это рекурсивно удалит все папки и файлы из данного каталага.
...
_Пояснение._
Если случалось удалить файл без использования `git rm`, обнаруживалось, что `git rm` всё равно придется использовать для удаленные файлов из рабочего дерева. Можете сохранить этот шаг с помощью опции "-а" в `git commit`, которая автоматически удаляет удаленные файлы с коммитами (к примеру, `git commit -a -m "Delete stuff"`)
...
_Пояснение._
Команда `git branch -d имя_ветки` работает только в случае, когда произошло объединение ветвей. Если хочется удалить ненужную ветвь без объединения, можно добавить любую из двух опций: опцию --force (-f), или использовать -D в сочетании с комбинаией -d -f в одной команде.
...
_Решение._
Мы уже определии что и куда поместить, а также сообщили Git, что  эти данные нужно запомнить. Так что теперь достаточно написать:
```
git push
```
