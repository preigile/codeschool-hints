...
_Пояснение._
Используя команду `git checkout имя\_ветви`, мы переключаемся между различными ветвями. Нам также известна команда для создания новой ветки: `git branch имя\_новой\_ветки`. Кроме того, мы можем, используя одну команду, добавить новую ветвь и одновременно переключиться на неё. Эта команда включает в себя две предыдущие: `git checkout -b имя\_новой\_ветки`.
...
_Пояснение._
Возможность удаления одного или всех файлов весьма полезна. Но иногда нужно удалить всю папку целиком. В этом случае можно использовать рекурсивную опцию на `git rm` - `git rm -r folder\_of\_cats`. Это рекурсивно удалит все папки и файлы из данного каталага.
